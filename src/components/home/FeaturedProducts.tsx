...
export function FeaturedProducts() {
  ...
  return (
    <section className="py-12 md:py-16 lg:py-24 bg-tikoun-black relative z-10 border-t border-tikoun-white/5">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
          {PRODUCTS.map((item, i) => (
            ...
            >
              {/* Product Card — 3D flip on desktop, direct CTA on mobile */}
              <div className="relative aspect-[4/5] mb-6 group/card">
                {/* Desktop: 3D Flip */}
                <div className="hidden md:block w-full h-full perspective-1000">
                ...
                {/* Mobile: Image + overlay CTA buttons */}
                <div className="md:hidden w-full h-full relative bg-tikoun-darkgray/30 rounded-2xl border border-tikoun-white/5 overflow-hidden">
                  {item.badge && (
                    <div className="absolute top-4 left-4 z-20 bg-tikoun-black/80 backdrop-blur-md border border-tikoun-gold/30 text-tikoun-gold px-3 py-1 text-[10px] font-bold tracking-widest uppercase rounded">
                      {item.badge}
                    </div>
                  ...
                  />
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-tikoun-black via-tikoun-black/80 to-transparent pt-16 pb-4 px-4 flex flex-col gap-2">
                    <Link
                      href={`/produits/${item.id}`}
                      className="w-full text-center border py-2.5 rounded-lg font-bold text-xs tracking-widest uppercase text-tikoun-white border-tikoun-white/30 active:bg-tikoun-white/10 transition-colors"
                    >
                      Voir les détails
                    </Link>
                    <button
                      ...
                      className="flex items-center gap-2 bg-tikoun-gold text-tikoun-black w-full justify-center py-2.5 rounded-lg font-bold text-xs tracking-widest uppercase active:bg-white transition-colors"
                    >
